package com.yema.mytestlibrary.utils
...
class MySpannableString(private val context: Context, text: CharSequence) : SpannableString(text) {
    ...
    fun size(dp: Int): MySpannableString {
        DensityUtil.px2dip(context, dp.toFloat())
    ...
    /**
     * 给target字串设置文字绝对大小为[px]
     */
    fun size2(px: Int): MySpannableString {
        for (range in rangeList) {
            setSpan(AbsoluteSizeSpan(DensityUtil.px2dip(context, px.toFloat()), true), range.first, range.second, spanMode)
        }
        return this
    }
